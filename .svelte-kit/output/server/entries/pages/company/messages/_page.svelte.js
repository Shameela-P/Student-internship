...
//#region src/routes/company/messages/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		derived(() => messages().filter((m) => m.senderEmail.toLowerCase() === company().companyEmail.toLowerCase() && m.recipientEmail.toLowerCase() === void 0 || m.senderEmail.toLowerCase() === void 0 && m.recipientEmail.toLowerCase() === company().companyEmail.toLowerCase()).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)));
		...
		$$renderer.push(`<!--]--></div></div> <div class="p-4 border-t border-slate-200 dark:border-slate-800/50 bg-slate-900/20 text-slate-500 text-[10px] font-bold">Connected as: ${escape_html(company().companyEmail)}</div></div> <div class="flex-grow flex flex-col justify-between bg-slate-950/20">`);
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<div class="flex-grow flex flex-col items-center justify-center text-slate-500 text-xs"><svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mb-3 text-slate-650" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z"></path></svg> Select a conversation from the sidebar to view candidate chats</div>`);
