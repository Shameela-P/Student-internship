...
function _layout($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		function getLinkClass(path) {
			if (path === "/company" ? page.url.pathname === path : page.url.pathname.startsWith(path)) return "flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition duration-200 cursor-pointer bg-indigo-600 text-slate-900 dark:text-white shadow-lg shadow-indigo-500/15";
		...
		if (unread() > 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<span class="absolute right-0 h-5 w-5 bg-rose-500 text-slate-900 dark:text-white rounded-full flex items-center justify-center text-[10px] font-bold">${escape_html(unread())}</span>`);
		} else $$renderer.push("<!--[-1-->");
