...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><h1 class="font-display font-black text-3xl text-primary dark:text-primary-dark tracking-tight">Manage Internship Postings</h1> <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Create new placement listings, update requirements, and review existing postings.</p></div> <button${attr("disabled", !approved(), true)} class="px-5 py-3 rounded-xl text-sm font-bold text-primary dark:text-primary-dark bg-blue-600 hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-500/10 disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="shrink-0"><line x1="12" x2="12" y1="5" y2="19"></line><line x1="5" x2="19" y1="12" y2="12"></line></svg> Publish Opportunity</button></div> `);
		if (form?.error) {
			$$renderer.push("<!--[0-->");
		...
		if (internships().length === 0) {
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<div${attr_class(`p-6 md:p-8 rounded-2xl bg-surface dark:bg-surface-dark/40 hover:bg-slate-900/60 border hover:border-blue-500/30 shadow-md hover:shadow-blue-500/5 hover:-translate-y-0.5 transition duration-300 flex flex-col justify-between ${isExpired ? "opacity-70 border-divider dark:border-divider-dark/50" : "border-divider dark:border-divider-dark/50"}`)}><div><div class="flex items-start justify-between"><div><div class="flex flex-wrap gap-1.5"><span class="px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider bg-blue-500/10 text-blue-400 border border-blue-500/20">${escape_html(intern.type)}</span> `);
				...
				$$renderer.push(`<!--]--></div></div> <div class="mt-6 pt-4 border-t border-divider dark:border-divider-dark/50 flex items-center justify-between"><span class="text-[10px] text-slate-500 font-bold">Deadline: ${escape_html(intern.lastDateToApply)}</span> <div class="flex items-center gap-2"><button class="px-3 py-1.5 rounded-lg border border-divider dark:border-divider-dark hover:bg-slate-800 text-[10px] font-bold text-slate-300 hover:text-primary dark:text-primary-dark transition cursor-pointer">Edit</button> `);
				if (intern.status === "Active") {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<form action="?/archiveInternship" method="POST"><input type="hidden" name="id"${attr("value", intern.id)}/> <button type="submit" class="px-3 py-1.5 rounded-lg border border-divider dark:border-divider-dark hover:bg-slate-800 text-[10px] font-bold text-amber-500 transition cursor-pointer">Archive</button></form>`);
				} else $$renderer.push("<!--[-1-->");
