...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (form && form.error || googleError) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="mb-6 p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 text-sm font-semibold flex items-center gap-2 shadow-sm animate-in fade-in slide-in-from-top-2"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><circle cx="12" cy="12" r="10"></circle><line x1="12" x2="12" y1="8" y2="12"></line><line x1="12" x2="12.01" y1="16" y2="16"></line></svg> ${escape_html(form?.error || googleError)}</div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> <form method="POST" class="space-y-6 w-full"><input type="hidden" name="role"${attr("value", activeRole)}/> <div><label for="email" class="block text-xs font-bold text-muted dark:text-muted-dark uppercase tracking-wider mb-2">`);
		$$renderer.push("<!--[-1-->");
