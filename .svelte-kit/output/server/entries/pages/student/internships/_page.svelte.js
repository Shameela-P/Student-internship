...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="mb-8"><h1 class="font-display font-black text-3xl text-slate-900 dark:text-white tracking-tight">Explore Placement Postings</h1> <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Search and apply for placement contracts across 150+ domains.</p></div> <div class="p-6 rounded-2xl bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800/80 mb-8 backdrop-blur-sm shadow-xl"><form method="GET" class="space-y-4"><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="relative"><input type="text" name="query" id="query"${attr("value", filters().query)} placeholder="Search titles, skills, or companies..." class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20"/> <svg class="absolute left-3 top-3 h-4 w-4 text-slate-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="11" cy="11" r="8" stroke-width="2"></circle><line x1="21" x2="16.65" y1="21" y2="16.65" stroke-width="2" stroke-linecap="round"></line></svg></div> <div><select name="domain" id="domain" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-300 focus:outline-none focus:border-blue-500">`);
		...
		$$renderer.push(`<!--]--></select></div> <div class="relative"><input type="text" name="location" id="location"${attr("value", filters().location)} placeholder="Filter by city/location..." class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"/> <svg class="absolute left-3 top-3 h-4 w-4 text-slate-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" stroke-width="2"></path><circle cx="12" cy="10" r="3" stroke-width="2"></circle></svg></div></div> <div class="grid grid-cols-2 lg:grid-cols-6 gap-3 pt-4 border-t border-slate-200 dark:border-slate-800/50"><div><select name="mode" id="mode" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-350 focus:outline-none focus:border-blue-500">`);
		$$renderer.option({ value: "" }, ($$renderer) => {
		...
		$$renderer.push(`</select></div> <div><select name="type" id="type" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-350 focus:outline-none focus:border-blue-500">`);
		$$renderer.option({ value: "" }, ($$renderer) => {
		...
		$$renderer.push(`</select></div> <div><select name="duration" id="duration" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-350 focus:outline-none focus:border-blue-500">`);
		$$renderer.option({ value: "" }, ($$renderer) => {
		...
		$$renderer.push(`</select></div> <div><select name="jobOpportunity" id="jobOpportunity" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-350 focus:outline-none focus:border-blue-500">`);
		$$renderer.option({ value: "" }, ($$renderer) => {
		...
		$$renderer.push(`</select></div> <div><select name="certificateAvailable" id="certificateAvailable" class="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 text-xs text-slate-350 focus:outline-none focus:border-blue-500">`);
		$$renderer.option({ value: "" }, ($$renderer) => {
		...
		$$renderer.push(`</select></div> <div class="col-span-2 lg:col-span-1 flex gap-2"><button type="submit" class="grow py-2.5 px-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white font-bold text-xs transition duration-200 cursor-pointer">Filter</button> <a href="/student/internships" class="px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800/50 hover:bg-slate-900 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white flex items-center justify-center cursor-pointer transition">Reset</a></div></div></form></div> `);
		if (internships().length === 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="p-16 rounded-3xl bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/80 text-center flex flex-col items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-slate-500 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg> <p class="text-sm font-bold text-slate-600 dark:text-slate-400">No matching internship listings found</p> <p class="text-xs text-slate-500 mt-1">Try relaxing filters or search queries.</p></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
			...
			for (let $$index_2 = 0, $$length = each_array_1.length; $$index_2 < $$length; $$index_2++) {
				...
				$$renderer.push(`<div class="group p-6 rounded-2xl bg-white dark:bg-slate-900/40 hover:bg-slate-900/60 border border-slate-200 dark:border-slate-800/50 hover:border-blue-500/30 shadow-md hover:shadow-blue-500/5 hover:-translate-y-1 transition duration-300 flex flex-col justify-between relative overflow-hidden"><div class="absolute top-0 right-0 w-20 h-20 rounded-full bg-gradient-to-bl from-blue-500/5 to-transparent blur-md pointer-events-none group-hover:scale-150 transition duration-500"></div> <div><div class="flex items-start justify-between"><div class="min-w-0 grow pr-2"><div class="flex flex-wrap items-center gap-1.5">`);
				...
				$$renderer.push(`<!--]--></div></div> <div class="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800/50 flex items-center justify-between"><span class="text-[10px] text-slate-500 font-bold">Apply by: <strong class="text-slate-350">${escape_html(intern.lastDateToApply)}</strong></span> `);
				if (intern.hasApplied) {
					...
					$$renderer.push("<!--[-1-->");
					$$renderer.push(`<button class="py-2 px-4 rounded-xl text-xs font-bold text-slate-900 dark:text-white bg-blue-600 hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-500/10 transition cursor-pointer">Apply Now</button>`);
				}
