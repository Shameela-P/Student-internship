...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="mb-8"><h1 class="font-display font-black text-3xl text-slate-900 dark:text-white tracking-tight">Interactive Alert Inbox</h1> <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Nexora email simulation inbox. Check automated emails, alerts, and registration notifications.</p></div> `);
		if (notifications().length === 0) {
			...
			for (let idx = 0, $$length = each_array.length; idx < $$length; idx++) {
				...
				$$renderer.push(`<!--]--> <span class="text-[10px] text-slate-600 dark:text-slate-400 font-semibold">${escape_html(new Date(notif.date).toLocaleString())}</span></div> <h3 class="font-display font-bold text-sm md:text-base text-slate-900 dark:text-white mt-2 truncate flex items-center gap-2">${escape_html(notif.subject)} `);
				if (!notif.read) {
					$$renderer.push("<!--[0-->");
