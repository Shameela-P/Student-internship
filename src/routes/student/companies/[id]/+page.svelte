...
<div class="mb-8 flex items-center justify-between">
	...
	<a href="/student/companies" class="px-4 py-2 rounded-xl border border-divider dark:border-divider-dark/80 bg-surface dark:bg-surface-dark/50 text-xs font-bold text-slate-600 dark:text-slate-400 hover:text-primary dark:text-primary-dark transition-all">
		← Back to Directory
	</a>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
	...
	<div class="lg:col-span-1 space-y-6">
		<div class="p-6 rounded-2xl bg-surface dark:bg-surface-dark/50 border border-divider dark:border-divider-dark/80 backdrop-blur-sm shadow-xl flex flex-col items-center text-center">
			{#if company.companyLogo}
				<img src={company.companyLogo} alt={company.companyName} class="w-24 h-24 rounded-2xl object-cover border border-slate-200 dark:border-slate-700/50 shadow-md mb-4" />
			{:else}
			...
			<h2 class="text-xl font-black text-primary dark:text-primary-dark font-display">{company.companyName}</h2>
			...
			<div class="w-full mt-6 pt-6 border-t border-divider dark:border-divider-dark/50 text-left space-y-4">
				{#if company.companyAddress}
					<div>
						<p class="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1">Location</p>
						<p class="text-sm font-semibold text-primary dark:text-primary-dark flex items-start gap-2">
		...
		<div class="p-6 rounded-2xl bg-surface dark:bg-surface-dark/50 border border-divider dark:border-divider-dark/80 backdrop-blur-sm shadow-xl">
			<h3 class="font-bold text-sm text-primary dark:text-primary-dark mb-4">Company Stats</h3>
			<div class="flex items-center justify-between">
				<span class="text-xs text-slate-600 dark:text-slate-400">Active Openings</span>
	...
	<div class="lg:col-span-2 space-y-8">
		<!-- About Section -->
		<section>
			<h3 class="text-lg font-black text-primary dark:text-primary-dark font-display mb-4 flex items-center gap-2">
			...
			<div class="p-6 rounded-2xl bg-surface dark:bg-surface-dark/50 border border-divider dark:border-divider-dark/80 shadow-md prose prose-sm dark:prose-invert max-w-none text-slate-700 dark:text-slate-300">
		...
		<section>
			<div class="flex items-center justify-between mb-4">
				<h3 class="text-lg font-black text-primary dark:text-primary-dark font-display flex items-center gap-2">
			...
			{#if internships.length === 0}
				<div class="p-12 rounded-2xl border border-divider dark:border-divider-dark border-dashed text-center bg-slate-50 dark:bg-slate-900/20">
					<p class="text-sm font-bold text-slate-500">No active postings right now.</p>
				</div>
			{:else}
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					{#each internships as intern}
						<div class="p-5 rounded-2xl bg-surface dark:bg-surface-dark/40 border border-divider dark:border-divider-dark hover:border-blue-500/30 transition-colors relative group">
							<a href="/student/internships?query={encodeURIComponent(intern.title)}" class="absolute inset-0 z-10 rounded-2xl"></a>
							
							<div class="flex items-start justify-between mb-3">
								<h4 class="font-bold text-primary dark:text-primary-dark text-sm pr-4 group-hover:text-blue-400 transition-colors">{intern.title}</h4>
							...
							<div class="mt-4 pt-3 border-t border-divider dark:border-divider-dark/50 flex items-center justify-between">
