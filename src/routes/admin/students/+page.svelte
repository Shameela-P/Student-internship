...
<div class="space-y-8">
	<!-- Header -->
	<div>
		<h1 class="text-3xl font-black font-display text-primary dark:text-primary-dark tracking-tight">Students Board</h1>
		<p class="text-muted dark:text-muted-dark mt-2">Manage and monitor all registered student accounts on the platform.</p>
	</div>

	<!-- Stats Cards -->
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
		<div class="glass-card rounded-2xl p-6 border-divider dark:border-divider-dark">
			<h3 class="text-xs font-bold text-muted dark:text-muted-dark uppercase tracking-wider mb-2">Total Students</h3>
			<p class="text-4xl font-black text-indigo-600 dark:text-indigo-400 font-display">{totalStudents.toLocaleString()}</p>
		</div>
		<div class="glass-card rounded-2xl p-6 border-divider dark:border-divider-dark">
			<h3 class="text-xs font-bold text-muted dark:text-muted-dark uppercase tracking-wider mb-2">Active Accounts</h3>
			<p class="text-4xl font-black text-emerald-600 dark:text-emerald-400 font-display">{activeAccounts.toLocaleString()}</p>
		</div>
		<div class="glass-card rounded-2xl p-6 border-divider dark:border-divider-dark">
			<h3 class="text-xs font-bold text-muted dark:text-muted-dark uppercase tracking-wider mb-2">Blocked</h3>
			<p class="text-4xl font-black text-rose-600 dark:text-rose-500 font-display">{blockedAccounts.toLocaleString()}</p>
		</div>
	</div>

	<!-- Search & Filters -->
	<div class="glass-card rounded-2xl p-4 border-divider dark:border-divider-dark flex flex-col md:flex-row gap-4 justify-between items-center">
		<div class="relative w-full md:w-96">
			...
			<input 
				...
				class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-divider dark:border-divider-dark bg-white/50 dark:bg-slate-900/50 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-400 text-sm transition"
			/>
		</div>
		<div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/60 p-1 rounded-xl border border-slate-200/5 dark:border-slate-800/40">
			{#each ['All', 'Active', 'Blocked'] as tab}
				<button 
					...
					class="px-4 py-2 rounded-lg text-xs font-bold transition duration-200 cursor-pointer {filterTab === tab ? 'bg-indigo-600 text-primary dark:text-primary-dark shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}"
				>
					{tab}
				</button>
			{/each}
	...
	<!-- Data Table -->
	<div class="glass-card rounded-2xl border-divider dark:border-divider-dark overflow-hidden">
		<div class="overflow-x-auto">
			<table class="w-full text-left border-collapse">
				<thead>
					<tr class="border-b border-slate-200/50 dark:border-slate-800/50 text-xs font-bold text-muted dark:text-muted-dark uppercase tracking-wider bg-slate-50/50 dark:bg-slate-900/30">
				...
				<tbody class="divide-y divide-slate-200/50 dark:divide-slate-800/50 text-sm">
					{#each filteredStudents.slice(0, 50) as student}
						<tr class="hover:bg-slate-50/50 dark:hover:bg-slate-100 dark:bg-slate-900/30 transition">
							<td class="px-6 py-4">
								<div class="font-bold text-primary dark:text-primary-dark">{student.fullName}</div>
								<div class="text-xs text-muted dark:text-muted-dark">{student.email}</div>
							</td>
					...
					{#if filteredStudents.length === 0}
						<tr>
							<td colspan="5" class="px-6 py-12 text-center text-muted dark:text-muted-dark">
