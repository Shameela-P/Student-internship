			...
			<div class="divide-y divide-slate-850/50 max-h-[68vh] overflow-y-auto">
				{#if threads.length === 0}
					<div class="p-8 text-center text-slate-500 text-xs font-semibold">
						No active message threads yet. Click the edit icon to compose a support chat.
					</div>
				{:else}
	...
	<div class="flex-grow flex flex-col justify-between bg-slate-950/20">
		{#if activeEmail}
			...
			<div class="p-4 border-b border-divider dark:border-divider-dark/50 bg-slate-100 dark:bg-slate-900/30 flex items-center justify-between">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<span class="text-[10px] text-blue-400 font-bold uppercase tracking-wider">{activeThread ? activeThread.role : 'User'}</span>
					</div>
				</div>
			</div>
			...
			<div class="flex-grow p-4 space-y-4 overflow-y-auto max-h-[58vh]">
				...
				{:else}
					{#each activeMessages as msg}
						{@const isSelf = msg.senderEmail.toLowerCase() === adminEmail.toLowerCase()}
			...
			>
				...
				<input type="hidden" name="recipientRole" value={activeThread ? activeThread.role : 'user'} />
		...
		{:else}
			<div class="flex-grow flex flex-col items-center justify-center text-slate-550 text-xs">
				...
				Select a conversation from the sidebar to view admin chats
			</div>
		{/if}
	</div>
</div>
